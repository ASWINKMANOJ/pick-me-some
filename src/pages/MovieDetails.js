...
import { movies, topMovies } from "../Data/MovieData";
...
const MovieDetail = () => {
  ...
  const movie =
    path[2] === "popular"
      ...
      : topMovies.find((movie) => movie.id === parseInt(id));
        ...
        <div className="flex flex-col w-full max-w-lg px-4 lg:px-8">
          ...
          <div className="mt-8">
            ...
            <ul className="list-disc space-y-2 text-gray-300 text-sm sm:text-base">
              <li>Director: {movie.detailed_info.director}</li>
              <li>Runtime: {movie.detailed_info.runtime}</li>
              <li>Genres: {movie.detailed_info.genres.join(", ")}</li>
              <li>
                Cast:{" "}
                {movie.detailed_info.main_cast.map((i) => i.name).join(", ")}
              </li>
            </ul>
          </div>
        </div>
...
export default pageTransition(MovieDetail);
